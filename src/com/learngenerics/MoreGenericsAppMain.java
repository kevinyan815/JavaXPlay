...
import com.learngenerics.ext.Children;
...
public class MoreGenericsAppMain {
    ...
    // >> TODO 逆变与协变相反，意思就是这个参数可以接受的List引用的泛型类型为Parent或者其父类
    ...
    public static void main(String[] args) {
        ...
        // >> TODO 下面这么传递参数不行，泛型类型不管继承关系，只管严格的匹配。
        // >> TODO 换句话说，Children 是 Parent 的子类，但是 List<Children> 不是 List<Parent> 的子类
//        justG2Method(g3List);

        // >> 那么怎么办呢？要让接收方的形参，声明为协变类型
        extMethod(g3List);// 这样就不会报错了
        ...
        // g2ListExt = new ArrayList<GrandParent>(); 超出Parent范围了，不行，会报错

        // >> TODO 但是使用这个带协变泛型的引用，我们无法让具体的类型满足其参数要求，打开下面注释掉的语句，编译时都会报错
...
//        g2ListExt.add(new Children());
        ...
        g2ListExt = g3OnlyList; // g2ListExt 引用的类型是List<? extends Parent>

        // >> TODO 这时候再看下一行，如果允许 add 一个Parent的对象，就代表着什么呢？
//        g2ListExt.add(new Parent());

        // >> TODO 那就代表着，原本只应该有Children或者其子类的g3OnlyList引用指向的对象，通过使用g2ListExt，被放进去一个Parent的实例
...
//        g2ListExt.add(new Object());
        ...
        // 怎么理解呢， 下面的引用变量声明类型是可以用协变和逆变，但是在new对象的时候不行， 创建对象的时候就得指明泛型要使用的具体类似
//         List<? extends Parent> g2ListExt11 = new ArrayList<? extends Parent>();
//         List<? super Parent> g2ListSup11 = new ArrayList<? super Parent>();
        List<? extends Parent> g2ListExt12 = new ArrayList<Children>();
        List<? super Parent> g2ListSup12 = new ArrayList<Parent>();
    }
}
